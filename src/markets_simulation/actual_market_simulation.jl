...
function create_realized_marketdata(simulation::AgentSimulation,
 ...
 =#
    ...
    energy_voll,
    reserve_voll,
    inertia_voll = energy_mkt_clearing(sys_UC, sys_ED, system, simulation_dir, reserve_penalty, rec_perc_requirement, zones, num_days, pcm_scenario, iteration_year, get_da_resolution(case), get_rt_resolution(case), get_name(case), solver, get_base_dir(case), simulation,current_siip_sim)
 ...
 =#
    ...
    @time Distributed.pmap(parallelize_ordc_construction, zip(all_scenarios, sys_UCs, data_dirs, investors_list, representative_periods_list, rep_period_intervals, cases, iteration_years, rolling_horizons, simulation_years_list))
